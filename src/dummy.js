...
const polybiusModule = (function () {
    ...
    function polybius(input, encode = true) {
      ...
      //decode
        ...
        //if(input.includes(" ")) {
        //  const noSpaceInput = input.toString().replaceAll(" ","")
        ...
        for (let i = 0; i < input.length; i += 2) {
          if (input[i] === " ") {
            ...
            const x = input[i]
            const y = input[i+1]
            const lettercoords = `${x}${y}`
            result += reverseLookup[lettercoords]
              //polybiusPairs.push(input.toLowerCase().substring(i,i+2))
          }
        }
       // polybiusPairs.forEach((pair)=>{
       //   let polybiusValue = reverseLookup[pair]
       //   result += polybiusValue
       // })
